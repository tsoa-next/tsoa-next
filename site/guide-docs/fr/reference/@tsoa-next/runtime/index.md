...
Ce paquet contient les helpers d'exécution pour `tsoa-next`.

`tsoa-next` poursuit l'original [`tsoa`](https://github.com/tsoa-next/tsoa-next) projet, et ce paquet fait avancer la fondation d'exécution établie là.

Pour la référence principale README et API, voir:
