...
Este pacote contém os ajudantes em tempo de execução para `tsoa-next`.

`tsoa-next` continua o original [`tsoa`](https://github.com/tsoa-next/tsoa-next) projeto, e este pacote leva adiante a fundação runtime estabelecida lá.

Para a referência principal README e API, consulte:
