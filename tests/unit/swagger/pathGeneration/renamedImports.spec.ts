...
describe('Renamed imports', () => {
  describe('runtime decorators', () => {
    ...
    it('should recognize aliased route and method decorators during metadata generation', () => {
      ...
      expect(metadata.controllers[0].methods.map(method => method.name)).to.deep.equal(['getAliased', 'postAliased'])
      expect(metadata.controllers[0].methods[0].method).to.equal('get')
      expect(metadata.controllers[0].methods[0].path).to.equal('{id}')
      expect(metadata.controllers[0].methods[1].method).to.equal('post')
      expect(metadata.controllers[0].methods[1].path).to.equal('body')
    })
    ...
    it('should resolve aliased imported models to one canonical schema name in OpenAPI 2', () => {
      ...
      expect(spec2.paths['/AliasedDecorators/{id}']?.get?.parameters).to.deep.include({
        in: 'path',
        name: 'id',
        required: true,
        type: 'string',
    ...
    it('should resolve aliased imported models to one canonical schema name in OpenAPI 3', () => {
      ...
      expect(spec3.paths['/AliasedDecorators/{id}']?.get?.parameters).to.deep.include({
        in: 'path',
        name: 'id',
        required: true,
        schema: {
